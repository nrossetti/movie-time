...
def save_setting(guild_id, key, value):
    directory = 'settings'
...
async def config(interaction, default_timezone: str = None, stream_channel: discord.VoiceChannel = None, announcement_channel: discord.TextChannel = None, ping_role: discord.Role = None, prefix: str = None):
    ...
    if default_timezone:
        timezone = US_TIMEZONE_ABBREVIATIONS.get(default_timezone.upper(), default_timezone)
        if timezone in pytz.all_timezones:
            save_setting(interaction.guild.id, 'default_timezone', timezone)
    ...
    if stream_channel:
        save_setting(interaction.guild.id, 'stream_channel', stream_channel.id)
        response_messages.append(f"Stream channel set to {stream_channel.mention}")

    if announcement_channel:
        save_setting(interaction.guild.id, 'announcement_channel', announcement_channel.id)
        response_messages.append(f"Announcement channel set to {announcement_channel.mention}")

    if ping_role:
        save_setting(interaction.guild.id, 'ping_role', ping_role.id)
...
@tree.command(name='post', description="Posts the most recent generated post.", guild=discord.Object(id=guild_id))
async def post(interaction):
    global latest_post_content

    announcement_channel_id = get_setting(interaction.guild.id, 'announcement_channel')
    ...
    if latest_post_content:
        ping_role_id = get_setting(interaction.guild.id, 'ping_role')
...
@tree.command(name="find_letterboxd_links", description="Finds all Letterboxd links in a specified channel's history.", guild=discord.Object(id=guild_id))
...
@tree.command(name="create_movie_post", description="Creates a movie post.", guild=discord.Object(id=guild_id))
async def create_post(interaction: discord.Interaction, start_time: str, theme_name: str, description: str, movie_urls: str):
    ...
    start_time_obj_time = parse_start_time(start_time)
    ...
    for index, url in enumerate(movie_urls_list, start=1):
        movie = movie_scraper.get_movie_details_from_url(url)
